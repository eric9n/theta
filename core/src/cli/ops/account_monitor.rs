...
use crate::ledger::{AccountMonitorSnapshotInput, Ledger};
...
pub struct Cli {
    ...
    #[arg(
        long,
        help = "Path to the portfolio database (default: ~/.theta/portfolio.db)"
    )]
    db: Option<PathBuf>,
...
async fn capture_once(ledger: &Ledger, account_id: &str, captured_at: &str) -> Result<()> {
    ...
    let account_snapshot = ledger
        ...
        .with_context(|| {
            format!("failed to load latest account snapshot for account {account_id}")
        })?
    ...
    let evaluated_strategies =
        margin_engine::evaluate_strategies(&strategies, &enriched, &account_ctx);
    ...
    println!(
        "{captured_at} account={account_id} quality=complete positions={} equity={:.2} pnl={:.2} margin={:.2}",
...
fn record_error_snapshot(
    ...
    let data_quality = classify_data_quality(err);
    ledger.record_account_monitor_snapshot(&AccountMonitorSnapshotInput {
        captured_at: captured_at.to_string(),
        account_id: account_id.to_string(),
    ...
    eprintln!("{captured_at} persisted error snapshot with quality={data_quality}");
    Ok(())
}

fn classify_data_quality(err: &anyhow::Error) -> &'static str {
    let message = format!("{err:#}");
    if message.contains("incomplete live quote coverage") {
        "incomplete_quotes"
    } else if message.contains("failed to fetch underlying quotes")
        || message.contains("failed to fetch option quotes")
    {
        "quote_unavailable"
    } else {
        "system_error"
...
mod tests {
    ...
    #[test]
    fn classify_data_quality_maps_incomplete_quote_errors() {
        let err = anyhow!("incomplete live quote coverage for positions: TSLA2401C");
        assert_eq!(classify_data_quality(&err), "incomplete_quotes");
    }

    #[test]
    fn classify_data_quality_maps_quote_fetch_errors() {
        let err = anyhow!("failed to fetch option quotes");
        assert_eq!(classify_data_quality(&err), "quote_unavailable");
    }

    #[test]
    fn classify_data_quality_defaults_to_system_error() {
        let err = anyhow!("sqlite is locked");
        assert_eq!(classify_data_quality(&err), "system_error");
    }
}
