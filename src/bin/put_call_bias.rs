...
use theta::signal_service::{PutCallBiasRequest, ThetaSignalService};
...
struct Cli {
    ...
    #[arg(long, default_value_t = 0.05, help = "Minimum OTM percent for OTM-only bias buckets")]
    min_otm_percent: f64,
    #[arg(long)]
    json: bool,
}
...
fn render_put_call_bias(view: &theta::domain::PutCallBiasView, json: bool) -> Result<()> {
    if json {
